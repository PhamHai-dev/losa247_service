...
import { ArrowRight, Bot, BrainCircuit, Building2, Camera, Check, CheckCircle2, ChevronDown, Clock3, CloudCog, Database, FileText, Filter, Globe2, GraduationCap, HeartPulse, Image, Layers3, MessageCircle, PlugZap, Quote, ScanLine, Send, ShieldCheck, ShoppingBag, Sparkles, Store, Tag, Truck, UserRoundCheck, Zap } from 'lucide-react';
...
const industries = [{ id: 'retail', icon: Store, name: 'Bán lẻ & E-commerce', accent: '#60a5fa', title: 'AI bán hàng từ tư vấn đến xác nhận đơn', text: 'Tìm đúng sản phẩm, kiểm tra tồn kho, tạo báo giá và hỗ trợ giao hàng trong một cuộc trò chuyện.', tasks: ['Gợi ý sản phẩm phù hợp', 'Kiểm tra giá và tồn kho', 'Tạo báo giá tức thì', 'Tính phí vận chuyển'], subject: 'Đơn hàng bán lẻ', subjectNote: 'Khách cần 20 máy lọc không khí', score: '92% phù hợp', steps: [['01', 'NHU CẦU', 'Đã xác định sản phẩm'], ['02', 'TỒN KHO', '20 sản phẩm sẵn sàng'], ['03', 'BÁO GIÁ', 'Đã tạo báo giá']], result: 'Cơ hội bán hàng sẵn sàng xử lý' }, { id: 'education', icon: GraduationCap, name: 'Giáo dục', accent: '#a78bfa', title: 'Tư vấn khóa học đúng nhu cầu từng học viên', text: 'AI sàng lọc mục tiêu, lịch học và ngân sách trước khi chuyển lead chất lượng cho tư vấn viên.', tasks: ['Tư vấn chương trình phù hợp', 'Sàng lọc mục tiêu học tập', 'Kiểm tra lịch khai giảng', 'Đặt lịch cùng tư vấn viên'], subject: 'Hồ sơ học viên', subjectNote: 'Quan tâm khóa IELTS buổi tối', score: 'Lead chất lượng', steps: [['01', 'MỤC TIÊU', 'IELTS 6.5'], ['02', 'LỊCH HỌC', 'Buổi tối trong tuần'], ['03', 'TƯ VẤN', 'Đã đặt lịch hẹn']], result: 'Học viên đã sẵn sàng tư vấn' }, { id: 'health', icon: HeartPulse, name: 'Y tế & phòng khám', accent: '#34d399', title: 'Tiếp nhận nhu cầu và điều phối lịch hẹn', text: 'Hỗ trợ thông tin dịch vụ, thu thập triệu chứng ban đầu và đặt lịch với đúng chuyên khoa.', tasks: ['Giải đáp thông tin dịch vụ', 'Thu thập nhu cầu ban đầu', 'Gợi ý chuyên khoa phù hợp', 'Nhắc lịch tự động'], subject: 'Yêu cầu đặt lịch', subjectNote: 'Khách cần tư vấn chuyên khoa', score: 'Đã xác minh', steps: [['01', 'NHU CẦU', 'Đã ghi nhận thông tin'], ['02', 'LỊCH TRỐNG', 'Có lịch phù hợp'], ['03', 'XÁC NHẬN', 'Đã gửi lịch hẹn']], result: 'Lịch hẹn đã được điều phối' }, { id: 'b2b', icon: Building2, name: 'Dịch vụ B2B', accent: '#fbbf24', title: 'Sàng lọc yêu cầu phức tạp trước khi tư vấn', text: 'Thu thập quy mô, bài toán và hệ thống hiện có để đội ngũ bước vào cuộc gọi với đầy đủ ngữ cảnh.', tasks: ['Khảo sát nhu cầu doanh nghiệp', 'Thu thập quy mô và hệ thống', 'Chấm điểm cơ hội', 'Đặt lịch cùng chuyên gia'], subject: 'Cơ hội doanh nghiệp', subjectNote: 'Yêu cầu tích hợp ERP nội bộ', score: 'Ưu tiên cao', steps: [['01', 'QUY MÔ', '200 nhân sự'], ['02', 'HỆ THỐNG', 'ERP và CRM riêng'], ['03', 'CHUYÊN GIA', 'Đã phân công']], result: 'Cơ hội B2B có đầy đủ ngữ cảnh' }];
...
function IndustryVisual({ item }) { const Icon = item.icon; return <div className="csp-industry-visual" style={{ '--industry-accent': item.accent }}><div className="csp-industry-visual__top"><span><Icon /></span><div><small>LOSA AI WORKSPACE</small><strong>{item.name}</strong></div><i><b /> Đang xử lý</i></div><div className="csp-industry-visual__card"><small>YÊU CẦU VỪA TIẾP NHẬN</small><div className="csp-industry-visual__subject"><span><Icon /></span><div><strong>{item.subject}</strong><small>{item.subjectNote}</small></div><b>{item.score}</b></div><div className="csp-industry-visual__steps">{item.steps.map(([no, label, value]) => <div key={no}><span>{no}</span><small>{label}</small><strong>{value}</strong><CheckCircle2 /></div>)}</div></div><div className="csp-industry-visual__result"><UserRoundCheck /><div><small>KẾT QUẢ</small><strong>{item.result}</strong></div><ArrowRight /></div></div> }
export default function ChatbotSolutionsPage() {
    const [cap, setCap] = useState('quote'), [industry, setIndustry] = useState('retail'), [faqOpen, setFaq] = useState(null); const q = useApiQuery(() => publicFaqsService.getList({ pageType: 'solutions', serviceDetail: 'chatbot' }), []); const fallback = [{ _id: 'accuracy', question: 'Chatbot AI trả lời sai thì xử lý thế nào?', answer: 'Losa giới hạn nguồn dữ liệu, thiết lập ngưỡng tin cậy và chuyển hội thoại cho nhân viên khi AI chưa đủ thông tin.' }, { _id: 'integration', question: 'Losa có kết nối CRM hoặc hệ thống riêng không?', answer: 'Có. Losa hỗ trợ đồng bộ CRM, dữ liệu sản phẩm và hệ thống nghiệp vụ thông qua API theo phạm vi triển khai.' }, { _id: 'launch', question: 'Mất bao lâu để triển khai chatbot?', answer: 'Thời gian phụ thuộc dữ liệu và mức độ tích hợp. Đội ngũ Losa sẽ khảo sát, chuẩn hóa và kiểm thử trước khi vận hành.' }]; const faqs = q.data?.items?.length ? q.data.items : fallback, active = caps.find(x => x.id === cap), usecase = industries.find(x => x.id === industry); useEffect(() => { document.title = 'Chatbot AI đa kênh cho doanh nghiệp | Losa'; let m = document.querySelector('meta[name="description"]'); if (!m) { m = document.createElement('meta'); m.name = 'description'; document.head.appendChild(m) } m.content = 'Chatbot AI Losa tự động tư vấn, báo giá, vận chuyển và chăm sóc khách hàng đa kênh.' }, []); return <main className="csp-page">
        <section className="csp-hero" id="chatbot-hero"><div className="csp-shell csp-hero__grid"><div className="csp-hero__copy"><div className="csp-eyebrow"><Sparkles /> CHATBOT AI ĐA KÊNH CHO DOANH NGHIỆP</div><h1>Mỗi cuộc trò chuyện là một <span>cơ hội tăng trưởng</span></h1><p>Chatbot AI Losa hiểu nhu cầu, tư vấn sản phẩm, tạo báo giá, tính phí vận chuyển và chăm sóc khách hàng 24/7 — trên mọi kênh bạn đang kinh doanh.</p><div className="csp-hero__actions"><button id="chatbot-view-demo" className="csp-btn csp-btn--primary" onClick={() => document.getElementById('capabilities')?.scrollIntoView({ behavior: 'smooth' })}>Khám phá năng lực AI <ArrowRight /></button><Link id="chatbot-hero-consult" className="csp-btn csp-btn--outline" to="/dang-ky">Đăng ký tư vấn</Link></div><div className="csp-hero__proof"><span><Check /> Phản hồi 24/7</span><span><Check /> Chuyển người thật khi cần</span><span><Check /> Dữ liệu tập trung</span></div></div><HeroVisual /></div></section>
        ...
        <section className="csp-section csp-usecases"><div className="csp-shell"><Heading eyebrow="AI THIẾT KẾ THEO NGÀNH" title="Mỗi doanh nghiệp có một cách vận hành khác nhau" text="Losa được thiết kế theo đúng quy trình, dữ liệu và tác vụ đặc thù của từng ngành." /><div className="csp-usecase-tabs" role="tablist">{industries.map(x => { const Icon = x.icon; return <button id={`industry-tab-${x.id}`} key={x.id} className={industry === x.id ? 'is-active' : ''} onClick={() => setIndustry(x.id)} role="tab" aria-selected={industry === x.id}><Icon />{x.name}</button> })}</div><div className="csp-usecase-panel" style={{ '--industry-accent': usecase.accent }}><div className="csp-usecase-panel__copy"><small>AI CHO {usecase.name.toUpperCase()}</small><h3>{usecase.title}</h3><p>{usecase.text}</p><div className="csp-usecase-tasks"><b>AI CÓ THỂ THỰC HIỆN</b>{usecase.tasks.map(x => <span key={x}><CheckCircle2 />{x}</span>)}</div><Link to="/dang-ky">Thiết kế AI cho doanh nghiệp <ArrowRight /></Link></div><IndustryVisual item={usecase} /></div></div></section>
