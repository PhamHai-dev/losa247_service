...
import {
  ...
  EditOutlined, DeleteOutlined, PhoneOutlined, MailOutlined, SearchOutlined, FilterOutlined
...
export function AdminLeads() {
  ...
  const initials = (name = '') => name.trim().split(/\s+/).slice(-2).map((part) => part.charAt(0)).join('').toUpperCase() || '?'
  const columns = [
    {
      title: 'Khách hàng', key: 'customer', width: 260, render: (_, lead) => (
        <div className="lead-identity">
          <Avatar size={44} src={lead.avatarUrl || lead.logoUrl} className="lead-avatar">{!(lead.avatarUrl || lead.logoUrl) && initials(lead.name)}</Avatar>
          <div className="lead-identity-copy"><strong>{lead.name || 'Chưa cập nhật tên'}</strong><span>{lead.email || 'Chưa có email'}</span></div>
        </div>
      ),
    },
    { title: 'SĐT', dataIndex: 'phone', width: 145, render: (phone) => phone ? <a className="lead-phone" href={`https://zalo.me/${phone.replace(/\D/g, '')}`} target="_blank" rel="noreferrer"><PhoneOutlined />{phone}</a> : '—' },
    { title: 'Nguồn', dataIndex: 'source', width: 120, render: (value) => <Tag className="lead-source">{sourceLabel(value)}</Tag> },
    {
      title: 'Nhân viên', dataIndex: 'assignedTo', width: 175, render: (user) => user ? (
        <div className="lead-owner"><Avatar size={28} src={user.avatarUrl}>{!user.avatarUrl && initials(user.name)}</Avatar><span>{user.name}</span></div>
      ) : <span className="lead-owner-empty">Chưa gán</span>,
    },
    { title: 'Trạng thái', dataIndex: 'status', width: 140, render: (value) => <StatusTag map={LEAD_STATUS} value={value} /> },
    { title: 'Cập nhật cuối', dataIndex: 'updatedAt', width: 140, render: (value) => <div className="lead-updated"><span>{dayjs(value).format('DD/MM/YYYY')}</span><small>{dayjs(value).format('HH:mm')}</small></div> },
    {
      title: '', key: 'actions', fixed: 'right', width: 120, render: (_, lead) => <div className="lead-row-actions">
        <Button type="text" icon={<EyeOutlined />} aria-label={`Xem ${lead.name}`} title="Xem chi tiết" onClick={() => setDetailId(lead._id)} />
        {canUpdate && <Button type="text" icon={<EditOutlined />} aria-label={`Sửa ${lead.name}`} title="Chỉnh sửa" onClick={() => openEdit(lead)} />}
        {canDelete && <Popconfirm title="Xóa Lead này?" onConfirm={() => handleDelete(lead._id)}><Button type="text" danger icon={<DeleteOutlined />} aria-label={`Xóa ${lead.name}`} title="Xóa" /></Popconfirm>}
      </div>,
    },
  ]
  const statItems = [
    { key: 'new', label: 'Lead mới', icon: <PlusOutlined />, color: '#0d9488', soft: '#e2f8f4' },
    { key: 'contacted', label: 'Đã liên hệ', icon: <PhoneOutlined />, color: '#0284c7', soft: '#e0f2fe' },
    { key: 'qualified', label: 'Tiềm năng', icon: <LikeOutlined />, color: '#d97706', soft: '#fff7df' },
    { key: 'converted', label: 'Đã chuyển đơn', icon: <CheckCircleOutlined />, color: '#16a34a', soft: '#e9f9ee' },
  ]
  ...
  return <section className="leads-page">
    ...
    <div className="leads-kpi-grid">
      {statItems.map((item) => <article className="leads-kpi" key={item.key} style={{ '--kpi-color': item.color, '--kpi-soft': item.soft }}><span className="leads-kpi-icon">{item.icon}</span><div className="leads-kpi-copy"><span>{item.label}</span><strong>{stats.byStatus?.[item.key] || 0}</strong><small><b className={stats.percentChange < 0 ? 'is-down' : ''}>{stats.percentChange >= 0 ? '+' : ''}{Number(stats.percentChange || 0).toFixed(1)}%</b> so với hôm qua</small></div></article>)}
    </div>

    <div className="leads-data-card">
      <div className="leads-toolbar">
        <div className="leads-toolbar-top"><div className="leads-toolbar-copy"><strong>Danh sách khách hàng</strong><span>{total} Lead trong hệ thống</span></div><FilterOutlined style={{ color: '#0d9488', fontSize: 18 }} /></div>
        <div className="leads-filter-grid">
          <Input className="leads-search" allowClear prefix={<SearchOutlined />} placeholder="Tìm tên, SĐT, email..." value={search} onChange={(event) => { onSearch(event.target.value); setPage(1) }} />
          <Select className="leads-filter-source" allowClear placeholder="Nguồn" value={filters.source || undefined} onChange={(value) => updateFilter('source', value || '')} options={sourceOptions} />
          <Select className="leads-filter-owner" allowClear showSearch optionFilterProp="label" placeholder="Nhân viên phụ trách" value={filters.assignedTo || undefined} onChange={(value) => updateFilter('assignedTo', value || '')} options={users.map((user) => ({ value: user._id, label: user.name }))} />
          <Select className="leads-filter-status" allowClear placeholder="Trạng thái" value={filters.status || undefined} onChange={(value) => updateFilter('status', value || '')} options={Object.entries(LEAD_STATUS).map(([value, item]) => ({ value, label: item.label }))} />
      ...
      <div className="leads-status-nav"><Segmented value={filters.status} onChange={(value) => updateFilter('status', value)} options={[{ value: '', label: `Tất cả (${stats.total || 0})` },...Object.entries(LEAD_STATUS).map(([value, item]) => ({ value, label: `${item.label} (${stats.byStatus?.[value] || 0})` }))]} /></div>

      {selectedRowKeys.length > 0 && <div className="leads-bulk-bar">
        <div className="leads-bulk-main"><div className="leads-bulk-count"><span className="leads-bulk-badge">{selectedRowKeys.length}</span><div className="leads-bulk-copy"><strong>Đã chọn {selectedRowKeys.length} Lead</strong><button type="button" onClick={() => setSelectedRowKeys([])}>Bỏ chọn</button></div></div>
          {canUpdate && <Select disabled={bulkLoading} placeholder="Đổi trạng thái" onChange={(value) => runBulkUpdate({ status: value })} options={Object.entries(LEAD_STATUS).map(([value, item]) => ({ value, label: item.label }))} value={undefined} />}
          {canAssign && <Select disabled={bulkLoading} showSearch optionFilterProp="label" placeholder="Gán nhân viên" onChange={(value) => runBulkUpdate({ assignedTo: value })} options={users.map((user) => ({ value: user._id, label: user.name }))} value={undefined} />}
          {canUpdate && <Select disabled={bulkLoading} placeholder="Đổi nguồn" onChange={(value) => runBulkUpdate({ source: value })} options={sourceOptions} value={undefined} />}
        </div>
        <div className="leads-bulk-actions">{canUpdate && <Button type="primary" icon={<MailOutlined />} onClick={() => setEmailOpen(true)}>Gửi email</Button>}{canExport && <Button icon={<DownloadOutlined />} onClick={handleExport}>Xuất Excel</Button>}{canDelete && <Popconfirm title={`Xóa ${selectedRowKeys.length} Lead đã chọn?`} onConfirm={handleBulkDelete}><Button danger icon={<DeleteOutlined />}>Xóa</Button></Popconfirm>}</div>
      </div>}

      {query.error && <Alert className="leads-error" type="error" showIcon message={query.error} />}
      <div className="leads-table-wrap"><Table rowKey="_id" loading={query.loading || bulkLoading} columns={columns} dataSource={rows} scroll={{ x: 1150 }} rowSelection={{ selectedRowKeys, onChange: setSelectedRowKeys, preserveSelectedRowKeys: true }} pagination={{ current: page, pageSize, total, onChange: setPage, showSizeChanger: false, showTotal: (count) => `${count} Lead` }} /></div>
    </div>

    <LeadDetailDrawer id={detailId} open={!!detailId} onClose={() => setDetailId(null)} onChanged={refresh} />
    <Modal className="lead-modal" title={editingLead ? 'Chỉnh sửa Lead' : 'Thêm Lead mới'} open={openForm} onOk={saveLead} onCancel={() => setOpenForm(false)} okText={editingLead ? 'Lưu thay đổi' : 'Thêm Lead'} cancelText="Hủy" destroyOnHidden>
    ...
    <Modal className="lead-modal" title={`Gửi email cho ${selectedRowKeys.length} Lead`} open={emailOpen} confirmLoading={bulkLoading} onOk={sendBulkEmail} onCancel={() => setEmailOpen(false)} okText="Gửi email" cancelText="Hủy" destroyOnHidden width={620}>
...
function LeadDetailDrawer({ id, open, onClose, onChanged }) {
  ...
  const configQuery = useApiQuery(() => settingsService.getLeadForm(), [], { enabled: open })
  const lead = query.data
  const dynamicItems = useMemo(() => {
    const values = lead?.customFields?.values || {}
    const labels = new Map((configQuery.data?.fields || []).map((field) => [field.key, field.label]))
    return Object.entries(values).filter(([key]) => !['name','phone','email'].includes(key)).map(([key,value]) => ({ key: `custom-${key}`, label: labels.get(key) || key, children: typeof value === 'boolean' ? (value ? 'Có' : 'Không') : String(value) }))
  }, [lead, configQuery.data])
