...
const createNextBatch = async (sessionId) => prisma.$transaction(async (tx) => {
  ...
  if (session.mode !== 'bot' || session.cycleLimitReachedAt) return { ignored: true, reason: 'BOT_DISABLED' };
  const active = await tx.chatBatch.findFirst({ where: { sessionId, botCycle: session.botCycle, status: { in: ACTIVE } }, orderBy: { createdAt: 'asc' } });
  if (active) return { deferred: true, batch: active };
  const messages = await tx.chatMessage.findMany({ where: { sessionId, botCycle: session.botCycle, sender: 'customer', batchId: null }, orderBy: [{ createdAt: 'asc' }, { id: 'asc' }], take: env.CHAT_BATCH_MESSAGE_LIMIT });
  if (!messages.length) {
    await tx.chatSession.update({ where: { id: sessionId }, data: { automationStatus: 'idle' } });
    return { empty: true };
  }
  const batch = await tx.chatBatch.create({ data: { id: createEntityId(), sessionId, sessionVersion: session.version, botCycle: session.botCycle, eventId: createEventId(), status: 'queued' } });
  await tx.chatMessage.updateMany({ where: { id: { in: messages.map((item) => item.id) }, batchId: null, botCycle: session.botCycle }, data: { batchId: batch.id, status: 'queued' } });
