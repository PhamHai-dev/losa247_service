...
const signAccessToken = (user, audience) => { const c = configFor(audience); return jwt.sign({ id: String(user._id), tokenType: 'access' }, c.secret, { algorithm: 'HS256', expiresIn: '15m', issuer: env.JWT_ISSUER, audience: c.audience, jwtid: crypto.randomUUID() }); };
const createRefreshToken = (user, audience, familyId = crypto.randomUUID()) => { const c = configFor(audience); const jti = crypto.randomUUID(); const token = jwt.sign({ id: String(user._id), tokenType: 'refresh', familyId }, c.secret, { algorithm: 'HS256', expiresIn: '7d', issuer: env.JWT_ISSUER, audience: c.audience, jwtid: jti }); return { token, jti, familyId, expiresAt: new Date(Date.now() + REFRESH_TTL_MS) }; };
...
const serializeUser = (user, extra = {}) => ({ id: user._id, name: user.name, email: user.email, phone: user.phone, role: user.role, status: user.status, avatarUrl: user.avatarUrl, ...extra });
